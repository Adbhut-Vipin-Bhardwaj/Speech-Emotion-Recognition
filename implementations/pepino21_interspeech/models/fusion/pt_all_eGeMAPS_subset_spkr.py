...
This model uses a subset of Low Level Descriptors (LLDs) of eGeMAPS ([2])
and all 13 layers of wav2vec 2.0. Concatenation is used for merging as given in
[1]. Uses speaker normalization.
...
class PreTrainedAllLayersSubsetOfLLDs(nn.Module):
    """
    Model using both eGeMAPS and wav2vec 2.0. Uses speaker normalization.
    Uses only a subset of eGeMAPS, input as a list.
    """
    def __init__(self,
                 max_seq_len,
                 dim1,
        ...
        eGeMAPS_feat_idxs: list of indices of which features of eGeMAPS are to
            be used.
        """
        super(PreTrainedAllLayersSubsetOfLLDs, self).__init__()
        ...
        # dict of means for feature normalization, or None
        # shape should be broadcastable to (*, len(eGeMAPS_feat_idxs))
        ...
        self.feat_idxs = eGeMAPS_feat_idxs
        ...
        self.pt_wise_conv1d_1_eGeMAPS = nn.Conv2d(
            in_channels=1,
            out_channels=dim1,
            kernel_size=(1, len(self.feat_idxs))
        )
    ...
    def forward(self, X):
        ...
        eGeMAPS_features = torch.zeros(
            (batch_size, self.max_seq_len, len(self.feat_idxs))
        )
        ...
        with torch.no_grad():
            for i in range(batch_size):
                ...
                eGeMAPS_feature_np = eGeMAPS_feature_np[:, self.feat_idxs]
                eGeMAPS_seq_len = eGeMAPS_feature_np.shape[0]
                if eGeMAPS_seq_len%2 == 0:
                    eGeMAPS_feature_np = eGeMAPS_feature_np\
                        .reshape((-1,2,len(self.feat_idxs))).mean(axis=1)
                else:
                ...
                if self.norm_means_w2v2 is not None:
                    w2v2_features[i, :, :int(lengths[i]), :] = (
                            w2v2_features[i, :, :int(lengths[i]), :]
                            - self.norm_means_w2v2[spkr_ids[i].item()]
                    )
                if self.norm_std_devs_w2v2 is not None:
                    w2v2_features[i, :, :int(lengths[i]), :] = (
                            w2v2_features[i, :, :int(lengths[i]), :]
                            / self.norm_std_devs_w2v2[spkr_ids[i].item()]
                    )
                if self.norm_means_eGeMAPS is not None:
                    eGeMAPS_features[i, :int(lengths[i]), :] = (
                            eGeMAPS_features[i, :int(lengths[i]), :]
                            - self.norm_means_eGeMAPS[spkr_ids[i].item()]
                    )
                if self.norm_std_devs_eGeMAPS is not None:
                    eGeMAPS_features[i, :int(lengths[i]), :] = (
                            eGeMAPS_features[i, :int(lengths[i]), :]
                            / self.norm_std_devs_eGeMAPS[spkr_ids[i].item()]
                    )
        ...
        # eGeMAPS_act_1 shape = (B,1,Tmax,len(self.feat_idxs))

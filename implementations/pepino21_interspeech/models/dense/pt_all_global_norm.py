...
pre trained wav2vec2 as feature extractor, uses uses all 13 layers (12 transformer
layers and 1 local encoding layer) of wav2vec2. Uses global normalization.
...
class PreTrainedAllLayersGlobalNorm(nn.Module):
    """
    Model as given in [1], using all 12 transformer layers
    with local encoder representations with pre-trained wav2vec2
    """
    def __init__(self,
                 max_seq_len,
                 norm_means=None,
                 norm_std_devs=None):
        super(PreTrainedAllLayersGlobalNorm, self).__init__()

        self.max_seq_len = max_seq_len
        # max length of wav2vec2 sequence (length in time)
        ...
        self.encodings = torch.zeros(13, self.max_seq_len, 768)
        self.seq_len = 0
        ...
        self.wtd_avg = nn.Conv2d(in_channels=13, out_channels=1, kernel_size=(1, 1), bias=False)
    ...
    def setup_hook(self, num):
        def hook(model, input, output):
            ...
            self.encodings[num:num+1, :self.seq_len, :]\
                = output.detach()[:, :self.seq_len, :]
        return hook
    ...
    def forward(self, X):
        ...
        features = torch.zeros((batch_size, 13, self.max_seq_len, 768))
        ...
        act_1 = self.wtd_avg(features)  # act_1 shape = (B,1,Tmax,768)

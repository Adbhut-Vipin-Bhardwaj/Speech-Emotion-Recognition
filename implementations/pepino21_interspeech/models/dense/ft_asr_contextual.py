...
wav2vec2 finetuned for ASR as feature extractor, uses 12 transformer layers of
wav2vec2. Uses speaker normalization.
...
class FineTunedAsrContLayers(nn.Module):
    """
    Model as given in [1], using 12 transformer layers only,
    (without local encoder representations) of fine tuned (for ASR) wav2vec2
    """
    def __init__(self,
                 max_seq_len,
                 norm_means=None,
                 norm_std_devs=None):
        super(FineTunedAsrContLayers, self).__init__()

        self.max_seq_len = max_seq_len
        # max length of wav2vec2 sequence
        self.norm_means = norm_means
        # dict of means for feature normalization, or None
        # shape should be broadcastable to (12, *, 768)
        ...
        self.encodings = torch.zeros(12, self.max_seq_len, 768)
        self.seq_len = 0
        for i in range(12):
            self.wav2vec2_model.encoder.transformer.layers[i].final_layer_norm\
                .register_forward_hook((self.setup_hook(i)))
            # transformer layers

        self.wtd_avg = nn.Conv2d(in_channels=12, out_channels=1, kernel_size=(1, 1), bias=False)
    ...
    def forward(self, X):
        ...
        features = torch.zeros((batch_size, 12, self.max_seq_len, 768))

...
wav2vec2 finetuned for ASR as feature extractor, uses all 13 layers (12 transformer
...
class FineTunedAsrAllLayers(nn.Module):
    """
    Model as given in [1], using all 12 transformer layers
    with local encoder representations of fine tuned (for ASR) wav2vec2
    """
    def __init__(self,
                 max_seq_len,
                 norm_means=None,
                 norm_std_devs=None):
        super(FineTunedAsrAllLayers, self).__init__()

...
layers and 1 local encoding layer) of wav2vec2. Uses speaker normalization.
...
class PreTrainedAllLayers(nn.Module):
    """
    Model as given in [1], using all 12 transformer layers
    with local encoder representations of pre-trained wav2vec2
    """
    def __init__(self,
                 max_seq_len,
                 norm_means=None,
                 norm_std_devs=None):
        super(PreTrainedAllLayers, self).__init__()
        ...
        nn.init.constant_(self.wtd_avg.weight, 1/13)
